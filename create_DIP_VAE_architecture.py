...
arch = [ 
    ...
    # deep4
    to_ConvConvRelu( name='ccr_b6', s_filer=16, n_filer=(128,128), offset="(1,0,0)", to="(pool_b3-east)", width=(6,6), height=16, depth=16 ),

    #Sampling
    # Encoder
    to_Linear(name='linear_b1_mu', offset="(1,10,0)", to="(ccr_b6-east)", s_filer=128*16*16, width=1, height=40, depth=1 ),
    to_Linear(name='linear_b2_mu', offset="(0,0,0)", to="(linear_b1_mu-east)", s_filer=8, width=1, height=4, depth=1 ),
    to_Linear(name='linear_b1_sigma', offset="(1,-10,0)", to="(ccr_b6-east)", s_filer=128*16*16, width=1, height=40, depth=1 ),
    to_Linear(name='linear_b2_sigma', offset="(0,0,0)", to="(linear_b1_sigma-east)", s_filer=8, width=1, height=4, depth=1 ),     

    to_Sampling(name='sampling', offset="(2,-10,0)", to="(linear_b2_mu-east)", radius=7),
    to_connection("ccr_b6","linear_b1_mu"), #sloped arg added in function
    to_connection("linear_b2_mu","sampling"), #sloped arg added in function
    r"""\draw [connection]  (linear_b2_mu-east)    -- node {\scalebox{2}{$\quad\mu$}} (sampling-west);""",
    to_connection("ccr_b6","linear_b1_sigma"), #sloped arg added in function
    to_connection("linear_b2_sigma","sampling"), #sloped arg added in function
    r"""\draw [connection]  (linear_b2_sigma-east)    -- node {\scalebox{2}{$\quad\sigma$}} (sampling-west);""",

    # Decoder
    to_Linear(name='linear_b3', offset="(1,0,0)", to="(sampling-east)", s_filer=2, width=1, height=2, depth=1 ),       
    to_connection("sampling","linear_b3"),

    #Decoder
    # up1
    to_UnPool(name="unpool_b1", offset="(1.5,0,0)", to="(linear_b3-east)", width=6, height=25, depth=25, opacity=0.5),
    to_Conv(name='ccr2_b6', offset="(0,0,0)", to="(unpool_b1-east)", s_filer=32, n_filer=64, width=4.5, height=25, depth=25 ),       
    to_connection("linear_b3","unpool_b1"),
